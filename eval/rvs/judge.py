...
def main():
    ...
    manifest = {"model": args.model, "revision": Path(model_path).name,
                ...
                "max_tokens": 64, "reasoning_effort": args.reasoning_effort, "backend": args.backend,
    ...
    if len(records) < len(rows):
        ...
        with checkpoint.open("a") as stream:
            for start in range(len(records), len(rows), args.batch_size):
                ...
                if args.backend == "transformers":
                    enc = tokenizer(prompts, return_tensors="pt", padding=True, truncation=True, max_length=4096).to(model.device)
                    with torch.inference_mode():
                        out = model.generate(**enc, do_sample=False, max_new_tokens=64, pad_token_id=tokenizer.eos_token_id)
                    generated = [tokenizer.decode(out[i, enc.input_ids.shape[1]:], skip_special_tokens=True) for i in range(len(batch))]
                else:
